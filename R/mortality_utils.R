...
calculate_roster_person_time_column <- function(
  data,
  start_col,
  end_col,
...
local_create_mortality_long_df <- function(
  df_main,
  date_dc = "today",
  date_recall_event = "recall_date",
  uuid_main = NULL,
  ...
  if (
    !all(
      c(date_dc, date_recall_event) %in%
        names(df_main)
    ) |
      length(c(date_dc, date_recall_event)) - 2 != 0
  ) {
    stop(
      "Check date_dc, date_recall_event arguments. Couldn't find in main data."
    )
  } else {
  ...
  if (all(req_cols %in% names(df_roster))) {
    ...
    stop(
      "Missing minimum information (SEX, AGE, Date of Birth) for current household roster. Please check input."
    )
  }
  ...
  if (all(req_cols %in% names(df_died))) {
    ...
    stop(
      "Missing minimum information (SEX, AGE, Dates of Birth or Death) for death roster. Please check input."
    )
  }
  ...
  tryCatch(
    {
      ...
      formula <- as.formula(paste0(
        rlang::as_string(outcome_col_sym),
        " ~ ",
        rlang::as_string(group_col_sym)
      ))
...
binomial_ratio_test_rowwise <- function(
  data,
  success_col,
  total_col,
  expected_ratio = 0.5,
...
t_test_rowwise_from_summary <- function(
  data,
  mean_col,
  sd_col,
  n_col,
...
poisson_ratio_test_rowwise <- function(
  data,
  event_col,
  exposure_col,
    ...
    if (!is.numeric(data[[col]])) {
      if (can_convert_to_numeric(data[[col]])) {
        ...
        warning(paste(
          "Column",
          col,
          "cannot be safely converted to numeric and will be excluded."
        ))
      }
    }
  }
    ...
    if (!is.numeric(data[[col]])) {
      if (can_convert_to_numeric(data[[col]])) {
        ...
        warning(paste(
          "Column",
          col,
          "cannot be safely converted to numeric and will be excluded from the summary."
        ))
      }
    }
  }
  ...
  for (var in numeric_vars) {
    ...
    if (valid_obs < 2) {
      warning(paste(
        "Skipping variable",
        var,
        "due to insufficient finite observations."
      ))
      next
    }
    ...
    cor_test <- tryCatch(
      ...
      error = function(e) {
        warning(paste(
          "Error in correlation test for variable",
          var,
          ":",
          e$message
        ))
    ...
    summary_table <- data %>%
      ...
      summarise(
        ...
        weighted_proportion = round(
          sum(.data[[weights_col]], na.rm = TRUE) /
            sum(data[[weights_col]], na.rm = TRUE),
          3
        ),
...
perform_chi_squared_tests <- function(
  data,
  count_var,
  categorical_vars,
  weight_var,
  ...
  for (var in categorical_vars) {
    ...
    for (dummy_var in colnames(dummy_data)) {
      ...
      chi_test <- tryCatch(
        {
          svychisq(
            as.formula(paste("~", count_var, "+", dummy_var)),
            survey_design
          )
        },
        error = function(e) {
          warning(paste(
            "Error in Chi-Square test for dummy variable",
            dummy_var,
            ":",
            e$message
          ))
...
required_sample_size_mmr <- function(
  expected_mmr = 500,
  desired_se = 100,
  crude_birth_rate = 35,
  recall_months = 12,
  ...
  cat(
    "Adjusted Crude Birth Rate:",
    round(adjusted_cbr, 2),
    "per 1,000 for recall period\n\n"
  )
  cat("Required number of live births:", round(required_births), "\n")
  cat(
    "Estimated population sample size:",
    round(total_population_required),
    "\n"
  )
  cat(
    "Estimated household sample size (pre-adjustment):",
    round(total_households_required),
    "\n"
  )
  cat("After design effect:", round(total_households_deff), "\n")
  cat(
    "Final household sample size (adjusted for non-response):",
    ceiling(final_household_sample),
    "\n"
  )
...
required_sisterhood_households <- function(
  ...
  maternal_deaths_per_sister_year <- (expected_mmr / 100000) *
    births_per_woman_year
  ...
  required_sister_years <- target_maternal_deaths /
    maternal_deaths_per_sister_year
  ...
  cat(
    "Target maternal deaths for this precision:",
    ceiling(target_maternal_deaths),
    "\n"
  )
...
render_quality_report <- function(
  main_data = NULL,
  roster_data = NULL,
  deaths_data = NULL,
  uuid_main,
